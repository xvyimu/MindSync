...
| 代码 tip | `develop` @ **以 `git log -1` 为准**（R2：A5 收口 + C2 导出） |
| 产品版本 | 2.11.7 |
...
## 7. 下一刀（冻结 · R3 表单 2026-07-21）

**方案 P1**：可交付工作台。  
规格：[`NEXT-CUT-SPEC-2026-07-21-E0-E1-E2.md`](./NEXT-CUT-SPEC-2026-07-21-E0-E1-E2.md)  
L0：`D:\PromtOptimizer\docs\INTEGRATED-WORKBENCH-DECISION-RESEARCH-2026-07-21-R3.md`

| 序 | 交付 | 状态 |
|----|------|------|
| E0 | 真机手测签字 | **todo**（清单已落盘） |
| E1 | 入口铺全 CTA+Eval+双模型（Basic+Context） | **代码 done** |
| E2 | 发版/NSIS + CURRENT | todo（建议手测后再说） |
| E3 | 手测缺陷 | 按需 |

实验 auto：**允许 UI 开关，默认关，非主 CTA**（§2.1 不变）。
...
|------|------|
| 主目标 | 本地工作台做深 |
| 硬约束 | 六条全维持 + 导出脱敏钉死 + Web 无 S3；自动优化见 §2.1 |
| 主输出 | push/A5 + F2 + C2 + C1（有序）· **已完成** |
...
|------|------|
| 主目标 | **P1 可交付工作台** |
| 硬约束 | 全维持；**允许实验 auto UI（默认关）** |
| 主输出 | E0 + E1 + E2 + E3 |
| E1 深度 | CTA + EvalCase + 双模型 |
| 竞品带回 | Context CTA（并入 E1） |
