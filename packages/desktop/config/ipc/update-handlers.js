  ...
  async function setupUpdateHandlers() {
    ...
    secureHandle(IPC_EVENTS.UPDATE_OPEN_RELEASE_PAGE, async (event, version) => {
      try {
        ...
        // MS-CR-003 + updater hardening: every release URL in this app is a GitHub
        // releases page (buildReleaseUrl and fallbackReleaseUrl both build one), so
        // enforce that exactly rather than the weaker "any http(s) with a hostname".
        // Two branches proposed both checks; the stricter one subsumes the other.
